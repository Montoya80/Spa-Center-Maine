...
import { User, Lock, ArrowRight, Smartphone, Eye, EyeOff, Calendar, ShieldCheck, KeyRound, X, Mail, MessageCircle, AlertCircle, ArrowLeft, UserPlus, UserX } from 'lucide-react';
...
const Login: React.FC<LoginProps> = ({ employees, patients, onLoginSuccess, onGuestBooking, initialTab = 'client', onBackToHome }) => {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (activeTab === 'employee') {
        ...
        if (foundEmpByPhone) {
            if (foundEmpByPhone.status === 'suspended') {
                setError('ACCESO DENEGADO: Tu usuario ha sido bloqueado por la administración.');
                return;
            }
            if (foundEmpByPhone.status === 'terminated') {
                setError('ERROR: Este usuario ya no forma parte del equipo activo.');
                return;
            }
        ...
        if (foundPatient) {
            if (foundPatient.isBlocked) {
                setError('CUENTA SUSPENDIDA: Tu acceso ha sido restringido. Por favor contacta a recepción.');
                return;
            }
                ...
                <form onSubmit={handleLogin} className="space-y-6">
                    ...
                    {error && (
                        <div className="bg-rose-50 text-rose-600 text-sm p-4 rounded-xl border border-rose-100 flex items-center gap-2 animate-fade-in">
                            <UserX size={18} className="shrink-0" /> <span className="font-bold">{error}</span>
                        </div>
                    )}
